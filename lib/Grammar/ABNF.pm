...
# RFC 5234 errata ID 2968
    ...
    token hex-val { "x" [ $<min>=[<.HEXDIG>+] "-" $<max>=[<.HEXDIG>+] |
                          [ $<val>=[<.HEXDIG>+] ]+ % "."]
                  }
...
my class ABNF-Actions {
    ...
    my sub guts($/) {
        ...
        for %*rules.pairs -> $rule {
            ...
            @parts.push('token ' ~ $rule.key ~ '{ ' ~ $rule.value ~ ' }');
        }
	$grmr.^add_method("FALLBACK", Grammar::ABNF.^find_method('FALLBACK'));
        $grmr.^compose;
        @parts.push('}');
        spurt $*name ~ '.generated', join("\n", @parts);
        make $grmr;
    }
	...
	# The .ords may get deprecated with NFG, and synthetic-leak-refusal
        ...
        make "[ " ~
             (~$/[0].comb.map({$_.ords}).fmt('\x%x', ' ').join)
              ~ " ]";
    }
